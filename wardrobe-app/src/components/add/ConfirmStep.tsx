...
export default function ConfirmStep(p: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, background: 'var(--bg)', paddingBottom: 100, overflowY: 'auto' }}>
      {/* Top bar */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 8px' }}>
        <button onClick={p.onRetake} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text)', fontSize: 14 }}>← Retake</button>
        <p style={{ margin: 0, fontWeight: 600, fontSize: 16 }}>New Piece</p>
        <button onClick={p.onDetails} disabled={!p.selectedCategory} style={{ background: 'none', border: 'none', cursor: p.selectedCategory ? 'pointer' : 'default', color: p.selectedCategory ? 'var(--text)' : 'var(--text-tertiary)', fontSize: 14, fontWeight: 600 }}>Details →</button>
      </div>
      ...
      <div style={{ padding: '0 16px 20px' }}>
        <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Category</p>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => {
              ...
              style={{ padding: '8px 16px', borderRadius: 999, fontSize: 14, fontWeight: 500, cursor: 'pointer', border: p.selectedCategory === cat ? '1px solid var(--text)' : '1px solid var(--border-strong)', background: p.selectedCategory === cat ? 'var(--text)' : 'var(--bg)', color: p.selectedCategory === cat ? 'var(--bg)' : 'var(--text)' }}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding: '0 16px 24px' }}>
        <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Colors</p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          {p.selectedColors.map(hex => {
            const preset = allPresets.find(pr => pr.hex === hex);
            return (
              <button key={hex} onClick={() => toggleColor(hex)}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px 6px 8px', borderRadius: 999, border: '1px solid var(--border-strong)', background: 'var(--bg)', cursor: 'pointer' }}>
                <div style={{ width: 18, height: 18, borderRadius: '50%', background: hex, border: isLightColor(hex) ? '1px solid #e5e7eb' : 'none', flexShrink: 0 }} />
                <span style={{ fontSize: 13, color: 'var(--text)' }}>{preset?.name ?? hex}</span>
                <span style={{ fontSize: 12, color: 'var(--text-tertiary)', marginLeft: 2 }}>×</span>
              </button>
            );
          })}
          <button onClick={() => setPaletteOpen(true)}
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 999, border: '1.5px dashed var(--text-tertiary)', background: 'var(--bg)', cursor: 'pointer', color: 'var(--text-secondary)', fontSize: 13 }}>
      ...
      {/* Save */}
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, padding: '12px 16px 28px', background: 'var(--bg)', borderTop: '1px solid var(--border)' }}>
        <button onClick={p.onSave} disabled={!p.selectedCategory}
          style={{ width: '100%', padding: '14px 0', borderRadius: 14, fontSize: 16, fontWeight: 600, border: 'none', cursor: p.selectedCategory ? 'pointer' : 'not-allowed', background: p.selectedCategory ? 'var(--text)' : 'var(--border-strong)', color: p.selectedCategory ? 'var(--bg)' : 'var(--text-tertiary)' }}>
          Save to Wardrobe
        </button>
      </div>
          ...
          <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, background: 'var(--bg)', borderRadius: '20px 20px 0 0', zIndex: 110, padding: '20px 16px 36px' }}>
              ...
              {allPresets.map(preset => {
                ...
                return (
                  <button key={preset.hex} onClick={() => { toggleColor(preset.hex); setPaletteOpen(false); }} title={preset.name}
                    ...
                    <div style={{ width: 40, height: 40, borderRadius: '50%', background: preset.hex, border: isLightColor(preset.hex) ? '1px solid #e5e7eb' : 'none', boxShadow: selected ? '0 0 0 3px var(--bg), 0 0 0 5px var(--text)' : '0 1px 3px rgba(0,0,0,0.15)' }} />
                    <span style={{ fontSize: 9, color: 'var(--text-secondary)', maxWidth: 44, textAlign: 'center', lineHeight: 1.2 }}>{preset.name}</span>
                  </button>
                );
              })}
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => customColorInputRef.current?.click()}
                style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 0', borderRadius: 12, border: '1px solid var(--border-strong)', background: 'var(--bg)', cursor: 'pointer', fontSize: 14, color: 'var(--text)' }}>
      ...
      {pendingCustomColor && (
        <>
          ...
          <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, background: 'var(--bg)', borderRadius: '20px 20px 0 0', zIndex: 130, padding: '24px 16px 36px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: 13, color: 'var(--text-secondary)' }}>This color will appear in your palette next time.</p>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => confirmSavePreset(true)}
                style={{ flex: 1, padding: '12px 0', borderRadius: 12, border: 'none', background: 'var(--text)', fontSize: 14, fontWeight: 600, cursor: 'pointer', color: 'var(--bg)' }}>
                Save to presets
              </button>
            </div>
          </div>
