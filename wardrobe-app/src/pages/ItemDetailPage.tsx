...
type ActiveSheet = 'category' | 'brand' | 'size' | 'material' | 'layer' | 'condition' | 'season' | 'notes' | 'measurements' | 'colors' | null;
...
export default function ItemDetailPage() {
  ...
  if (!item) {
    return (
      <div style={{ display: 'flex', flex: 1, alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12 }}>
        ...
        <button onClick={() => navigate('/wardrobe')} style={{ fontSize: 14, color: '#374151', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Back to wardrobe</button>
      </div>
    );
  }

  // Tappable row that opens a bottom sheet
  ...
  const sheetBackdrop = (
    <div onClick={cancelSheet} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100 }} />
  );

  const sheetContainer = (children: React.ReactNode) => (
    <>
      {sheetBackdrop}
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, background: 'white', borderRadius: '20px 20px 0 0', zIndex: 110, padding: '20px 16px 36px', maxHeight: '70vh', overflowY: 'auto' }}>
    ...
    sheetContainer(
      <>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {options.map(opt => (
            <button key={opt} onClick={() => { onSelect(opt === selected ? null : opt); confirmSheet(); }}
  ...
  const notesSheet = () =>
    sheetContainer(
      <>
        ...
        <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Anything worth remembering…" rows={4} autoFocus
  ...
  const measurementsSheet = () => {
    ...
    return sheetContainer(
      <>
        <p style={{ fontSize: 15, fontWeight: 600, margin: '0 0 14px' }}>Measurements</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 14 }}>
          {fields.map(label => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 14, color: '#6b7280', width: 120, flexShrink: 0 }}>{label}</span>
              <input type="number" value={measurements[label] ?? ''} onChange={e => setMeasurements(prev => ({ ...prev, [label]: e.target.value }))} placeholder="—"
              ...
              <span style={{ fontSize: 12, color: '#9ca3af', width: 30 }}>cm</span>
            </div>
          ))}
        </div>
        <button onClick={confirmSheet} style={{ width: '100%', padding: '12px 0', borderRadius: 12, border: 'none', background: '#111827', color: 'white', fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>Done</button>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, background: 'white', overflowY: 'auto', paddingBottom: 100 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', padding: '16px 16px 8px', position: 'sticky', top: 0, background: 'white', zIndex: 10 }}>
        ...
        <div style={{ justifySelf: 'end', display: 'flex', gap: 12, alignItems: 'center' }}>
          <button onClick={handleDelete} disabled={deleting} aria-label="Delete item" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, opacity: deleting ? 0.5 : 1 }}>
      ...
      <div style={{ position: 'relative', padding: '0 0 20px' }}>
        <img src={item.imageUrl} alt="clothing item" style={{ width: '100%', height: 320, objectFit: 'cover', display: 'block' }} />
        <button onClick={toggleFavorite} aria-label={item.isFavorite ? 'Unfavorite' : 'Favorite'}
          style={{ position: 'absolute', bottom: 28, right: 12, background: 'none', border: 'none', cursor: 'pointer', padding: 8, filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.3))' }}>
        ...
        {colors.length > 0 ? (
          <div style={{ display: 'flex', gap: 8, flex: 1, flexWrap: 'wrap', minWidth: 0 }}>
            {colors.map(hex => {
              ...
              return (
                <div key={hex} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 18, height: 18, borderRadius: '50%', background: hex, border: hex.toLowerCase() === '#ffffff' ? '1px solid #e5e7eb' : 'none', flexShrink: 0 }} />
      ...
      {activeSheet === 'material' && multiChipSheet('Material', MATERIALS, material, setMaterial)}
      {activeSheet === 'layer' && chipSheet('Layer', LAYERS, layer, setLayer)}
        ...
        return sheetContainer(
          <>
            ...
            {customColors.length > 0 && (
              <>
                <p style={{ fontSize: 12, color: '#9ca3af', margin: '0 0 10px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Custom</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
                  {customColors.map(hex => {
                    const selected = colors.includes(hex);
                    return (
                      <button key={hex} onClick={() => setColors(prev => selected ? prev.filter(c => c !== hex) : [...prev, hex])} title={hex}
                        style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: hex, boxShadow: selected ? '0 0 0 3px white, 0 0 0 5px #111827' : '0 1px 3px rgba(0,0,0,0.15)' }} />
                        <span style={{ fontSize: 9, color: '#6b7280', maxWidth: 44, textAlign: 'center', lineHeight: 1.2 }}>{hex}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 16 }}>
              {DEFAULT_COLOR_PRESETS.map(preset => {
                const selected = colors.some(c => c.toLowerCase() === preset.hex.toLowerCase());
                return (
                  <button key={preset.hex} onClick={() => setColors(prev => selected ? prev.filter(c => c.toLowerCase() !== preset.hex.toLowerCase()) : [...prev, preset.hex])} title={preset.name}
                    style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                    <div style={{ width: 36, height: 36, borderRadius: '50%', background: preset.hex, boxShadow: selected ? '0 0 0 3px white, 0 0 0 5px #111827' : '0 1px 3px rgba(0,0,0,0.15)', border: preset.hex === '#ffffff' ? '1px solid #e5e7eb' : 'none' }} />
